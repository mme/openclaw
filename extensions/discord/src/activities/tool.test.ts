...
import {
  buildDiscordWidgetDocument,
  createDiscordWidgetTool,
  DISCORD_WIDGET_HTML_MAX_BYTES,
} from "./tool.js";
...
describe("discord_widget", () => {
  ...
  it("keeps full documents unchanged and rejects oversized HTML", async () => {
    ...
    expect(buildDiscordWidgetDocument("Full", document)).toBe(document);
    const tool = createDiscordWidgetTool(discordContext(), {
      runtime: createActivityTestRuntime(),
    ...
    await expect(
      tool.execute("oversized", {
        html: "x".repeat(DISCORD_WIDGET_HTML_MAX_BYTES + 1),
        title: "Too large",
      }),
    ).rejects.toThrow(`html exceeds maximum size (${DISCORD_WIDGET_HTML_MAX_BYTES} bytes)`);
  });
