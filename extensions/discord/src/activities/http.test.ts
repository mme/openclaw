...
import { createDiscordActivityHttpHandler, DISCORD_ACTIVITY_WIDGET_CSP } from "./http.js";
...
describe("Discord Activity widget routes", () => {
  ...
  it("resolves a custom ID in the validated instance channel and consumes its doc token", async () => {
    ...
    expect(firstDocument.headers.get("content-security-policy")).toBe(DISCORD_ACTIVITY_WIDGET_CSP);
    ...
    expect(secondDocument.headers.get("content-security-policy")).toBe(DISCORD_ACTIVITY_WIDGET_CSP);
  });
