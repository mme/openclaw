...
<Steps>
  ...
  <Step title="Copy the OAuth2 client secret">
    Open **OAuth2** in the Developer Portal. Add `https://127.0.0.1` as a placeholder redirect URI if the application has no redirect yet; Discord requires one, while the Embedded App SDK handles the Activity return flow. Copy or reset the application client secret. Treat it as a credential: do not paste it into chat, logs, or a committed configuration file.
  </Step>
